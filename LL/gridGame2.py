...
class grid(object):
    ...
    def droneBestMove(self):
        moved = False
        distGrid = self.pathFind(self.visionGrid, self.wallGrid, self.playerPos)[:]
        x, y = self.dronePos
        if y > 0:
            up = distGrid[x][y-1]
        else:
    ...
    def canMove(self, wallGrid):
        if self.pathFind(self.visionGrid, wallGrid, self.dronePos)[self.playerPos[0]][self.playerPos[1]] == 99:
            return -1
        else:
            return 1
